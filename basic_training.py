...
parser.add_argument('--size', choices=['small', 'large', 'transfer'])
...
att_type = 'fm' if args.fm else 'cm'
prefix = f"{args.dataset}_{args.size}_{args.layer}_{att_type}"
...
def model_fn(dataset, size):
    ...
    if dataset=='cifar':
        ...
        shape = (100, 100, 3)
        n_classes = 2
    if size=='transfer' and not args.pretrained:
        print("using resnet! rerun with --pretrained")
        exit()
        vgg = tf.keras.applications.ResNet50(include_top=False, input_shape=shape, pooling='avg')
    else:
...
def gradient_match(model_path, pert_x, pert_y, targ_x, targ_y):
    ...
    for _ in range(its):
        ...
        pert_x = np.clip(new_val, orig_pert_x - 1/100, orig_pert_x + 1/100)
        tf.keras.backend.set_value(tf_x, pert_x)
    print(np.linalg.norm((pert_x - orig_pert_x).reshape(pert_x.shape[0], -1), axis=1).mean())
    
    return pert_x

pretrained_path = "/net/data/pois_priv/subclass_avail_cifar/{args.dataset}_pretrained_{args.size}"

if not args.pretrained:
    print("always use pretrained")
    exit()
...
pois_rates = [0.5, 1, 2]  # pois_rates = [0.5, 1, 2]
...
for fake_index in range(1):
    ...
    for cl_ind in cl_to_att:
        ...
        #px_gm = gradient_match(pretrained_path, p_x, p_y, img_aux, aux_y)
        ...
        model = train_model(args.dataset, args.size, p_trn_x, p_trn_y, img_tst, targ_tst)
    ...
    np.save(prefix + "_" + str(args.layer) + "_errs_" + str(trn_ind)+'_'+str(pois_rate), errs)
    np.save(prefix + "_" + str(args.layer) + "_cleans_" + str(trn_ind)+'_'+str(pois_rate), collats)
